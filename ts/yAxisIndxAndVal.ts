export function yAxisBarIndxAndVal() {
  const barDivs: NodeListOf<HTMLDivElement> = document.querySelectorAll('.bar');
  ...
  for(let i = 0; i < barDivs.length; i++) {
    ...
    if((barValue * 2) % 2 == 1)                                   // if decimal value
      (barDivs[i].firstChild as HTMLSpanElement).style.right = "-25px";

    if(i >= 9)                                                      
      barDivs[i].style.setProperty("--bar-number-left", "-20px");
    
    if(barValue == 10)
      (barDivs[i].firstChild as HTMLSpanElement).style.right = "-18px";
